...
void draw_rect(struct limine_framebuffer *fb, uint32_t width, uint32_t height, uint32_t x, uint32_t y, uint32_t color){
     for (int i = 0; i < y; i++){
        for(int j = 0; j < x; j++){
...
void draw_char(struct limine_framebuffer *fb, int x, int y, char c, uint32_t color, int scale){
    if (c < 0 || c > 127)
        return;
